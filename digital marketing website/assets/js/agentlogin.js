...
//     script.src = `https://script.google.com/macros/s/AKfycbzB9F4g68ZKkWnEN1wXA4BBTFXztXssO-VwK3YH87vPlUK7piQkcQwJd58-TtpwRhCn/exec?
//     action=checkDuplicates&password=${encodeURIComponent(
//       password
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  button.addEventListener("click", async function (event) {
    ...
    // 5️⃣ Inject JSONP script to check duplicates
    const script = document.createElement("script");
    script.src = `https://script.google.com/macros/s/AKfycbzB9F4g68ZKkWnEN1wXA4BBTFXztXssO-VwK3YH87vPlUK7piQkcQwJd58-TtpwRhCn/exec?action=checkDuplicates&password=${encodeURIComponent(password)}&referral=${encodeURIComponent(referral)}&callback=${callbackName}`;
    document.body.appendChild(script);
  });
});
...
async function loginUser(username, password) {
  try {
    ...
    console.error("🔥 An error occurred while checking validity of data", error);
...
document.getElementById("submit-log").addEventListener("click", async (e) => {
  e.preventDefault();
